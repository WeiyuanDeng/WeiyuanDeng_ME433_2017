...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
        {
            ...
            float accelaration_y;
            //            float acc_z;

            // get IMU data
            I2C_read_multiple(IMU_ADDR, 0x20, imu_data, 14);

            acceleration_x = ((float) (get_acc_x(imu_data)))*0.061 / 100;
            acceleration_y = ((float) (get_acc_y(imu_data)))*0.061 / 100 - 1;


            if (c == 5) {
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                appData.xCoordinate = (int8_t) (acceleration_x);
                appData.yCoordinate = (int8_t) (acceleration_y);
                c = 0;
            } else {

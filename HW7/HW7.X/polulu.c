...
#include"polulu.h" // high-level IMU library
#include"ILI9163C.h"
...
void drawxXLvec(char x, char y, signed char magnitude, short done) { // draw x-vector
    int barX, barY;
    if (magnitude > 0) { // if magnitude is positive
        for (barX=0; barX<50; barX++) {
            if (barX<magnitude) {
                ...
                for (barY=0; barY<4; barY++) {
                    LCD_drawPixel(x+barX,y+barY,BLACK);
                }
            }
            for (barY=0; barY<4; barY++) {
                    LCD_drawPixel(x-barX,y+barY,BLACK); // erase the bar in the opposite direction
            }
        }
    }
...
void drawyXLvec(char x, char y, signed char magnitude, short done) { // draw y-vector
    int barX, barY;
    if (magnitude > 0) { // if magnitude is positive
